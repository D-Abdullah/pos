<div class="popup-add popup close shadow-sm rounded-3 position-fixed">
    ...
    <form id="add-cate" method="post" action="{{ route('employee.add') }}">
        @csrf
        <h2 class="text-center mt-4 mb-4 opacity-75">اضافة موظف جديد</h2>
        <div class="f-row d-flex gap-4">
            <div>
                <label class="d-block mb-1" for="customer-name">اسم الموظف</label>
                <input class="category-input" type="text" name="name" id="customer-name" placeholder="اسم الموظف">
            </div>
            <div>
                <label class="d-block mb-1" for="category">رقم الهاتف</label>
                <input class="category-input" type="number" minlength="11" name="phone" id="category"
