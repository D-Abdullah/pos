...
@section('title', 'تعديل فاتورة الحفله')
...
@section('content')
    <!-- start of body -->
    <h2 class="mb-2 mt-5">تعديل فاتوره الحفله "{{ $party->name }}"</h2>
    ...
    <table id="table" class="table w-100 mb-4 border">
        ...
        <tbody id="dataTableBody">
            ...
            @foreach ($bills as $i => $bill)
                <tr id="row-{{ $i }}" class="{{ $bill->type == 'eol' ? 'type-eol' : '' }}">
                    <td class="text-center" style="vertical-align: middle;">
                        @if ($bill->from == 'custom')
                            <img src="{{ asset('imgs/default.png') }}" width="75" height="75"
                                style="object-fit: cover; border-radius: 25px">
                        @elseif ($bill->from == 'product')
                            <img src="{{ asset($bill->product->image) }}" width="75" height="75"
                                style="object-fit: cover; border-radius: 25px">
                        @elseif ($bill->from == 'rent')
                            <img src="{{ asset($bill->rent->image) }}" width="75" height="75"
                                style="object-fit: cover; border-radius: 25px">
                        @else
                            <img src="{{ asset('imgs/default.png') }}" width="75" height="75"
                                style="object-fit: cover; border-radius: 25px">
                        ...
                        <div class="edit d-flex align-items-center justify-content-center">
                            @if ($bill->from == 'custom')
                                ...
                                <img src="{{ asset('Assets/imgs/trash (1).png') }}" alt=""
                                    class="ms-2 me-2 open-modal" data-type="{{ $bill->from }}" data-id=""
                                    data-action="delete">
                            @elseif ($bill->from == 'product')
                                <img src="{{ asset('Assets/imgs/edit-circle.png') }}" alt="" class="open-modal"
                                    data-type="{{ $bill->from }}" data-id="{{ $bill->product_id }}"
                                    data-action="edit">
                                <img src="{{ asset('Assets/imgs/trash (1).png') }}" alt=""
                                    class="ms-2 me-2 open-modal" data-type="{{ $bill->from }}"
                                    data-id="{{ $bill->product_id }}" data-action="delete">
                            @elseif ($bill->from == 'rent')
                                <img src="{{ asset('Assets/imgs/edit-circle.png') }}" alt="" class="open-modal"
                                    data-type="{{ $bill->from }}" data-id="{{ $bill->rent_id }}" data-action="edit">
                                <img src="{{ asset('Assets/imgs/trash (1).png') }}" alt=""
                                    class="ms-2 me-2 open-modal" data-type="{{ $bill->from }}"
                                    data-id="{{ $bill->rent_id }}" data-action="delete">
                            @else
                                -
                            @endif
                        </div>
        ...
        <tfoot>
            <tr>
                ...
                <td colspan="6">
                    <form action="{{ route('party.updateBill', $id) }}" method="post" id="requestBillForm">
                        @csrf
                        @method('PUT')
                        <button id="payButton" class="submit main-btn p-3 ps-4 pe-4 w-100" style="cursor: no-drop;"
                            disabled type="submit">تأكيد معلومات
                            الفاتوره</button>
                        @if ($bills->count() > 0)
                            @foreach ($bills as $i => $bill)
                                <input type="hidden" name="bill[{{ $i }}][id]" value="{{ $bill->id }}">

                                <input type="hidden" name="bill[{{ $i }}][from]"
                                    value="{{ $bill->from }}">
                                <input type="hidden" name="bill[{{ $i }}][name]"
                                    value="{{ $bill->name }}">
                                <input type="hidden" name="bill[{{ $i }}][quantity]"
                                    value="{{ $bill->quantity }}">
                                <input type="hidden" name="bill[{{ $i }}][unit_price]"
                                    value="{{ $bill->unit_price }}">
                                <input type="hidden" name="bill[{{ $i }}][total_price]"
                                    value="{{ $bill->total_price }}">
                                <input type="hidden" name="bill[{{ $i }}][type]"
                                    value="{{ $bill->type }}">
                                <input type="hidden" name="bill[{{ $i }}][status]"
                                    value="{{ $bill->status }}">
                                <input type="hidden" name="bill[{{ $i }}][party_id]"
                                    value="{{ $id }}">
                                <input type="hidden" name="bill[{{ $i }}][product_id]"
                                    value="{{ $bill->product_id }}">
                                <input type="hidden" name="bill[{{ $i }}][rent_id]"
                                    value="{{ $bill->rent_id }}">
                                <input type="hidden" name="bill[{{ $i }}][eol_reason]"
                                    value="{{ $bill->eol_reason }}">
                            @endforeach
                        @endif
                    </form>
                </td>
...
@section('script')
    ...
    <script type="text/javascript" defer src="{{ asset('Assets/JS files/editBill.js') }}"></script>
@endsection
