...
@section('title', 'انواع المعاملات الماليه')
...
@section('content')
    ...
    <h2 class="mt-5 mb-5">انواع المعاملات الماليه</h2>
    ...
    <section class="pt-0 rounded-3 position-relative shadow-sm overflow-auto">
        ...
        <div class="features shadow-sm p-4 d-flex justify-content-between align-items-end">
            ...
            <div class="component-right d-flex align-items-end">
                ...
                <div class="add-button">
                    <button class="text-light main-btn ms-4"> اضافه نوع معامله جديد</button>
                </div>
        ...
        <table class="w-100 mb-4 border" id="table">
            ...
            <thead class="head">
                <tr>
                    <th>اسم نوع المعامله الماليه</th>
                    <th>بواسطه</th>
                    <th>التاريخ</th>
                    <th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="up">
                                ...
                                <span class="fw-bold">{{ $ftType->added_by }}</span>
                            </div>
                        </td>
                        <td> {{ $ftType->updated_at->format('Y/m/d') }}</td>

                        <td>
                            <div class="edit d-flex align-items-center justify-content-center">
                                <img src="{{ asset('Assets/imgs/edit-circle.png') }}" data-id="{{ $ftType->id }}"
                                    id="edit">

                                <img class="ms-2 me-2" src="{{ asset('Assets/imgs/trash (1).png') }}"
                                    data-id="{{ $ftType->id }}" alt="" id="trash">

                            </div>
                        </td>
                        <td class="p-0">
                            <div class="popup-edit  id-{{ $ftType->id }} popup close shadow-sm rounded-3 position-fixed">
                                <form id="edit-cate" method="post" action="{{ route('ft.type.update', $ftType->id) }}">
                                    @csrf
                                    @method('put')
                                    <img class="position-absolute" src="{{ asset('Assets/imgs/Close.png') }}"
                                        alt="">
                                    <h2 class="text-center mt-4 mb-4 opacity-75">تعديل: {{ $ftType->name }} </h2>
                                    <div class="f-row d-flex gap-4">
                                        <div>
                                            <label class="d-block mb-1" for="customer-name">اسم نوع المعامله الماليه</label>
                                            <input class="category-input" type="text" name="name"
                                                id="customer-name" value="{{ $ftType->name }}"
                                                placeholder="اسم نوع المعامله الماليه
">
                                        </div>
                                    </div>
                                    {{-- <div class="form-check form-switch d-flex align-items-center  ms-2 me-2">
                                        <input class="form-check-input ms-3"
                                            @if ($ftType->is_active) checked @endif type="checkbox"
                        ...
                        <div class="popup-delete popup close shadow-sm rounded-3 position-fixed">
                            ...
                            <div class="buttons mt-5 d-flex">
                                <button onclick="fnDelete('{{ $ftType->id }}')" class="agree rounded-2">نعم
                                    أريد</button>
        ...
        <div class="table-control d-flex justify-content-between align-items-center">
            <div class="buttons-div">
                @if ($ftTypes->currentPage() != 1)
                    <a href="{{ $ftTypes->previousPageUrl() }}" class="p-2 rounded-3 bg-primary text-white">السابق</a>
                @endif

                @for ($i = max(1, $ftTypes->currentPage() - 2); $i <= min($ftTypes->lastPage(), $ftTypes->currentPage() + 2); $i++)
                    <a href="{{ $ftTypes->url($i) }}"
                        class="number-pages text-light ms-1 me-1 main-btn {{ $i == $ftTypes->currentPage() ? 'bg-primary' : '' }}">{{ $i }}</a>
                @endfor

                @if ($ftTypes->currentPage() != $ftTypes->lastPage())
                    <a href="{{ $ftTypes->nextPageUrl() }}" class="p-2 rounded-3 bg-primary text-white">التالي</a>
                @endif
            </div>

            <div class="info-table opacity-50">
                <p>عرض <span>{{ $ftTypes->firstItem() }}</span> إلى <span>{{ $ftTypes->lastItem() }}</span>
                    من
                    <span>{{ $ftTypes->total() }}</span> مدخلات
                </p>
            </div>
        </div>
...
@section('script')
    ...
    <script>
        ...
        function fnDelete(id) {
            ...
            form.setAttribute('action', `{{ url('financial-transaction-type') }}/${id}`);
