        ...
        <div class="features shadow-sm p-4 d-flex justify-content-between align-items-end">
            ...
            <div class="component-right d-flex align-items-end">
                ...
                <div class="add-button">
                    <button class="text-light main-btn ms-4"> اضافه خزنه</button>
                </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="up">
                                ...
                                <span class="fw-bold">{{ $safe->added_by }}</span>
                            </div>
                        </td>
                        <td> {{ $safe->updated_at->format('Y/m/d') }}</td>

                        <td>
                            <div class="edit d-flex align-items-center justify-content-center">
                                <img src="{{ asset('Assets/imgs/edit-circle.png') }}" data-id="{{ $safe->id }}"
                                    id="edit">

                                <img class="ms-2 me-2" src="{{ asset('Assets/imgs/trash (1).png') }}"
                                    data-id="{{ $safe->id }}" alt="" id="trash">

                            </div>
                        </td>
                        <td class="p-0">
                            <div class="popup-edit  id-{{ $safe->id }} popup close shadow-sm rounded-3 position-fixed">
...
">
                        ...
                        <div class="popup-delete popup close shadow-sm rounded-3 position-fixed">
                            <img class="position-absolute normal-dismiss" src="{{ asset('Assets/imgs/Close.png') }}"
                                alt="">
                            <h3 class="fs-5 fw-bold mb-3">حذف الخزنه</h3>
                            <p>هل تريد الحذف متاكد !!</p>
                            <div class="buttons mt-5 d-flex">
                                <button onclick="fnDelete('{{ $safe->id }}')" class="agree rounded-2">نعم
                                    أريد</button>
        ...
        <div class="table-control d-flex justify-content-between align-items-center">
            <div class="buttons-div">
                @if ($safes->currentPage() != 1)
                    <a href="{{ $safes->previousPageUrl() }}" class="p-2 rounded-3 bg-primary text-white">السابق</a>
                @endif

                @for ($i = max(1, $safes->currentPage() - 2); $i <= min($safes->lastPage(), $safes->currentPage() + 2); $i++)
                    <a href="{{ $safes->url($i) }}"
                        class="number-pages text-light ms-1 me-1 main-btn {{ $i == $safes->currentPage() ? 'bg-primary' : '' }}">{{ $i }}</a>
                @endfor

                @if ($safes->currentPage() != $safes->lastPage())
                    <a href="{{ $safes->nextPageUrl() }}" class="p-2 rounded-3 bg-primary text-white">التالي</a>
                @endif
            </div>

            <div class="info-table opacity-50">
                <p>عرض <span>{{ $safes->firstItem() }}</span> إلى <span>{{ $safes->lastItem() }}</span>
                    من
                    <span>{{ $safes->total() }}</span> مدخلات
                </p>
            </div>
        </div>
...
@section('script')
    ...
    <script>
        ...
        function fnDelete(id) {
            ...
            form.setAttribute('action', `{{ url('safe') }}/${id}`);
