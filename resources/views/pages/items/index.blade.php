                    ...
                    <tr>
                        ...
                        <td>
                            <div class="edit d-flex align-items-center justify-content-center">
                                ...
                                @can('delete product')
                                    <img class="ms-2 me-2" src="{{ asset('Assets/imgs/trash (1).png') }}" data-id="{{$product->id}}"
                                         alt=""
                                         id="trash">
                                @endcan
                            </div>
                        ...
                        @can('update product')
                                ...
                                <form method="post" action="{{route('product.update', $product->id)}}">
                                    ...
                                    <div class="f-row d-flex gap-4">
                                        <div>
                                            ...
                                            <input type="text" name="name" value="{{$product->name}}" id="category-name"
                                            ...
                                            <div class="select-form">
                                                ...
                                                <select class="form-control form-select" id="department" name="department_id">
                                                    @foreach($departments as $department)
                                                        <option value="{{$department->id}}" @if($product->department_id == $department->id) selected @endif>{{$department->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                            ...
                            {{$product->description}}</textarea>
                                    ...
                                    <div class="form-check form-switch d-flex align-items-center  ms-2 me-2">
                                        <input class="form-check-input ms-3" @if($product->is_active) checked
                                               @endif type="checkbox" role="switch"
                                               id="flexSwitchCheckDefault-97" name="is_active" value="1">
                        ...
                        @can('delete product')
                            <div
                                class="popup-delete id-{{$product->id}} popup close shadow-sm rounded-3 position-fixed">
                                <img class="position-absolute" src="{{ asset('Assets/imgs/Close.png') }}" alt="">
            ...
            <div class="table-control d-flex justify-content-between align-items-center">
                ...
                <div class="info-table opacity-50">
                    <p>عرض <span>{{ $products->firstItem() }}</span> إلى <span>{{ $products->lastItem() }}</span>
                        من
                        <span>{{ $products->total() }}</span> مدخلات</p>
                </div>
            </div>
        @endcan
