@if(Session::has('warn'))
    <style>
        ...
        .warning {
            background-color: var(--preparing-background);
    ...
    <div class="warning p-3">
        <div class="up d-flex justify-content-between align-items-center">
            <div class="data d-flex align-items-center">
                <img src="{{asset('Assets/imgs/bill-Alert.png')}}" alt="">
                <p class="me-3 fw-bold fs-4">{{Session::get('warn')}}</p>
            </div>
            <img src="{{asset('Assets/imgs/error-x.png')}}" alt="">
        </div>
    </div>

    <script>
        document.querySelector(".warning .up > img").addEventListener("click", (e) => {
